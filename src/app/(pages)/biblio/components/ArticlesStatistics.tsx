...
const ArticleStatistics: React.FC = () => {
  ...
  const includedArticles = useMemo(
    () => biblio.filter((article: Article) => article.DO_INCLUDE),
    [biblio]
  );
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-4 mt-4 fixed top-0 left-0 ml-2">
      <h2 className="text-2xl font-bold text-primary mb-1">
        Selected Article : {includedArticles.length}
      </h2>
      <h5 className="text-small text-secondary mb-4">
        PDF found : {includedArticles.filter((a) => a.hasPdf).length}
      </h5>
      <div>
