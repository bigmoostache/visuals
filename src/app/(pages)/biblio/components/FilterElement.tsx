...
const FilterElement: React.FC<FilterElementProps> = ({
  ...
  useEffect(() => {
    ...
    // @ts-ignore
    uniqueValues = Array.from(new Set(articles.map((article) => article[fieldId])))
    .filter((value) => value !== undefined && value !== null);

    const valueOptions = uniqueValues
      .map((value) => {
        if (typeof value === "boolean") {
          return { value: value ? 1 : 0, text: value.toString() };
        } else {
  ...
  const handleSort = () => {
    const newSortOrder =
      sortOrder === undefined || sortOrder === "asc" ? "desc" : "asc";
    onSort(fieldId, newSortOrder);
  };

  const isNumber =
    typeof values[0]?.value === "number" ||
    typeof values[0]?.value === "boolean" || type === 'boolean' || type==='number';

  return (
    <div
      className={`m-2 p-4 bg-white rounded-lg shadow-md ${
        isNumber ? "w-[23%]" : "w-[48%]"
      }`}
    >
      <div className="flex justify-between items-center mb-2">
        ...
        <div className="inline relative cursor-pointer" onClick={handleSort}>
          <FaSortUp
            className={`mb-[-10px] ${
              sortOrder === "asc" ? "text-secondary" : "text-primary"
            }`}
          />
          <FaSortDown
            className={`mt-[-10px] ${
              sortOrder === "desc" ? "text-secondary" : "text-primary"
            }`}
          />
        </div>
      </div>
